...
import sys
import os
from augmentation.bbox_util import *
import torch
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, sample):
            ...
            if random.random() < self.p:
                img = image[:, ::-1, :]
                annots[:, [0, 2]] += 2*(img_center[[0, 2]] - annots[:, [0, 2]])

                box_w = abs(annots[:, 0] - annots[:, 2])
...
class Rotate(object):
    ...
    def __call__(self, sample):
        ...
        corners = get_corners(annots)
        
        corners = np.hstack((corners, annots[:,4:]))
        ...
        annots = clip_box(annots, [0,0,w, h], 0.25)
...
class RandomHSV(object):
    ...
    def __init__(self, hue = None, saturation = (-10,10), brightness = (-10,10)):
        if hue:
            self.hue = hue 
        else:
            self.hue = 0
